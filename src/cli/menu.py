...
async def run_main_menu(session_manager, run_loop_cb, sync_cb, settings_repo):
    while True:
        ...
        if choice == _("Exit") or choice is None:
            ...
            console.print(
                _("[cyan]Started session for: {name}[/cyan]").format(
            ...
            if action == _("Run search"):
                console.print(
                    _("[cyan]Running session: {name}[/cyan]").format(
                ...
                confirm = await questionary.confirm(
                    _(
                        "Are you sure? This will delete the session '{name}' and {count} linked offers."
                    ).format(name=selected_session.display_name, count=count)
                ).ask_async(kbi_msg=_("\nCancelled by user\n"))

                if confirm:
                    await session_manager.delete_session(selected_session.id)
                    console.print(
                        _("[green]Session '{name}' deleted.[/green]").format(
                            name=selected_session.display_name
                        )
                    )
            ...
            console.print(
                _("[yellow]'{choice}' is not implemented yet.[/yellow]").format(
                    choice=choice
                )
            )
