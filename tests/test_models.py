...
def test_compute_fingerprint_normalization():
    ...
    offer1 = Offer(
        title="Beautiful Apartment! ", description="  Very nice, clean.", price=price1
    )
    offer2 = Offer(
        title="beautiful apartment", description="very nice clean", price=price2
    )
...
def test_compute_fingerprint_handles_nones():
    offer = Offer(title="Minimal Offer")
